...
function Swipe(container, options) {
  ...
  var events = {
    ...
    start: function(event) {
      // Reacher add for before callback
    ...
    move: function(event) {
      ...
      if (!isScrolling) {
        ...
        if (isValidSlide && !isPastBounds) {
          ...
          options.callback && options.callback(index, slides[index],flags);

        } else {
          flags = direction ? "left" : "right";


          (Math.abs(delta.x)>20 || Math.abs(delta.x) > width/2) && options.callback && options.callback(index, slides[index],flags);
